...
RUN apt-get update && \
    apt-get install --no-install-recommends -y chromium-browser curl git gnupg libgbm1 \
    gconf-service libxshmfence1 libasound2 libatk1.0-0 libatk-bridge2.0-0 \
    ...
    #&& apt-get update \
    #&& apt-get install --no-install-recommends -y google-chrome-stable fonts-ipafont-gothic fonts-wqy-zenhei fonts-thai-tlwg fonts-kacst fonts-freefont-ttf libxss1
